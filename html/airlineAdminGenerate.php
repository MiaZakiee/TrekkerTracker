...
<body>
    ...
    <main class="d-flex flex-nowrap">
        <div class="d-flex flex-column flex-shrink-0 p-3 bg-body-tertiary" style="width: 280px; height: 100vh;">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li>
                    <a href="./airlineAdminGenerate.php" class="nav-link active" aria-current="page">
                        <svg class="bi pe-none me-2" width="16" height="16">
                            <use xlink:href="#grid" />
                        </svg>
                        Generate flights
    ...
    <script>
        function addAirlineField() {
            // Create a new input element
            var newField = document.createElement('input');
            newField.type = 'text';
            newField.className = 'form-control dynamicField';
            newField.name = 'airline[]'; // Use array notation for name to handle multiple values

            // Add the new input field to the 'airlineFields' div
            document.getElementById('airlineFields').appendChild(newField);
        }
        ...
        function addLocationFields() {
            ...
            newField.type = 'text';
            newField.className = 'form-control dynamicField';
            newField.name = 'location[]'; // Use array notation for name to handle multiple values

            // Add the new input field to the 'airlineFields' div
            document.getElementById('locationFields').appendChild(newField);
        }
    </script>
...
if (isset($_POST['generateFlights'])) {
    ...
    $span = 0;
    if (isset($_POST['daySpan'])) {
        $span = $_POST['daySpan'];
        echo $span;
    }

    if ($airlines == null || $locations == null || $span == 0) {
        return;
    }
    ...
    for ($i = 0; $i < $span; $i++) {
        ...
        while ($flightTime < $endTime) {
            // Select random places for flight
            $randLocA = rand(0, sizeof($locations) - 1);
            $randLocB = rand(0, sizeof($locations) - 1);
            while ($randLocA == $randLocB) {
                $randLocB = rand(0, sizeof($locations) - 1);
            }
